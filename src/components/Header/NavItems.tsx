...
const NavItems: FC<Props> = ({ isMobileTablet }) => {
    ...
    <>
      {isMobileTablet ? (
        <div className='flex items-center flex-col'>
          <div
            className='w-full items-center relative cursor-pointer text-[17px] font-Poppins font-medium text-[#e0e0e0]'
            onClick={() => {
          ...
          >
            <div className='flex items-center justify-between h-[40px] px-2'>
              ...
              <FaSortDown fill='#00dc5a' />
            </div>

            {openCatModal && (
              <>
                <ul className='w-full py-1 px-2 bg-[#1a191f] grid grid-cols-2 sm:grid-cols-3 justify-items-center'>
                  {navItemsData.map((nav, index) => (
                    <Link
                      key={index}
                      to={`/danh-sach/${nav.slug}?page=1`}
                      className='block text-[14px] sm:text-[16px] font-Poppins font-medium text-[#e0e0e0] drop-shadow-[1px_1px_1px_#000] hover:text-[#00dc5a] p-2 text-left'
                    >
                      {nav.name}
                    </Link>
                  ))}
          ...
          >
            <div className='flex items-center justify-between h-[40px] px-2'>
              <span className='ml-1'>Thể Loại</span>
              <FaSortDown fill='#00dc5a' />
            </div>
            ...
            {openGenreModal && (
              <>
                <ul className='w-full py-1 px-2 bg-[#1a191f] grid grid-cols-2 sm:grid-cols-3 justify-items-center'>
                  {genreItemsData.map((genre, index) => (
                    <Link
                      key={index}
          ...
          >
            <div className='flex items-center justify-between h-[40px] px-2'>
              <span className='ml-1'>Quốc Gia</span>
              <FaSortDown fill='#00dc5a' />
            </div>
            ...
            {openNationModal && (
              <>
                <ul className='w-full py-1 px-2 bg-[#1a191f] grid grid-cols-2 sm:grid-cols-3 justify-items-center'>
                  {nationItemsData.map((nation, index) => (
                    <Link
                      key={index}
